...
//     // ✅ Handle case when documents is sent as JSON string (form-data)
...
//     for (const submittedDoc of documents) {
//       const field = documentFields.find((f) => String(f._id) === submittedDoc.fieldId);
...
//     user.documents = Array.from(
//       new Set([...(user.documents as Types.ObjectId[]), ...savedDocIds])
//     );
//     await user.save();

//     // Fetch a lean user object to avoid Mongoose metadata in the response
//     const leanUser = await User.findById(userId).lean();
//     if (!leanUser) {
//       sendResponse(res, null, "User not found", STATUS_CODES.NOT_FOUND);
//       return;
...
//     // Find all UserDocument models for the user
//     const userDocs = await UserDocument.find({ user: userId }).lean();
...
//     // Replace documents array on user object
//     leanUser.documents = responseDocuments;
...
export const approveDocumentStatus = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const { userId, fieldId, documentName, status } = req.body;
    const adminId = req.user?.id;

    if (req.user?.role !== "admin") {
      sendResponse(res, null, "Unauthorized: Admin access required", STATUS_CODES.UNAUTHORIZED);
      return;
    }

    if (!mongoose.Types.ObjectId.isValid(userId) || !mongoose.Types.ObjectId.isValid(fieldId)) {
      sendResponse(res, null, "Invalid user ID or field ID", STATUS_CODES.BAD_REQUEST);
      return;
    }

    if (!documentName || !["approved", "rejected"].includes(status)) {
      sendResponse(res, null, "Invalid document name or status", STATUS_CODES.BAD_REQUEST);
      return;
    }
    
    const updatedDoc = await UserDocument.findOneAndUpdate(
      { user: userId, "values._id": new mongoose.Types.ObjectId(fieldId), "values.name": documentName },
      {
        $set: { "values.$.status": status }
      },
      { new: true, lean: true } // Return the updated document and make it lean
    );
    
    if (!updatedDoc) {
      sendResponse(res, null, "Document not found for the specified user and field", STATUS_CODES.NOT_FOUND);
      return;
    }
    
    sendResponse(res, updatedDoc, "Document status updated successfully", STATUS_CODES.OK);
  } catch (error) {
    next(error);
  }
};
